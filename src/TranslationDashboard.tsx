...
import { writeTextFile, getRpyFiles, readRpyFile } from './tauriApi';
import { pyString } from './rpyValue';
...
interface LangEntry {
  ...
  imported?: boolean;
  /** The game's tl/ folder for this language (e.g. `schinese`), when it has one. */
  dir?: string;
}
...
const TranslationDashboard: React.FC<TranslationDashboardProps> = ({ project, rootPath, onProjectChange }) => {
  ...
  // Writes the language's translations to game/tl/<lang>/vnv_translations.rpy
  const exportRpy = useCallback(async () => {
    if (!currentLang || !rootPath) {
      ToastManager.error(t('toasts.sync_no_root'));
      return;
    }
    // The game's own folder for the language, so its language picker finds the translations
    const langSlug = currentLang.dir ?? activeLang.toLowerCase().replace(/\s+/g, '_');

    // Ren'Py stops if a string is translated twice, so strings the game's own
    // translation files (e.g. an imported game's) already cover are left out.
    const covered = new Set<string>();
    try {
      for (const file of await getRpyFiles(rootPath)) {
        if (!file.startsWith(`game/tl/${langSlug}/`) || file.endsWith('/vnv_translations.rpy')) continue;
        for (const text of translatedStrings(await readRpyFile(`${rootPath}/${file}`))) covered.add(text);
      }
    } catch (err) {
    ...
    for (const s of strings) {
      ...
      if (covered.has(s.original)) { skipped++; continue; }
      if (written.has(s.original)) continue; // repeated text shares one translation
      written.add(s.original);
      lines.push(
        `    # ${s.sceneName} — ${s.type}`,
        `    old ${pyString(s.original)}`,
        `    new ${pyString(translated)}`,
        ``,
      );
    }
    ...
    try {
      ...
      if (skipped) ToastManager.info(t('toasts.sync_skipped').replace('{count}', String(skipped)).replace('{lang}', langSlug));
    } catch (err: any) {
      ToastManager.error(t('toasts.sync_failed').replace('{err}', String(err)));
    }
