...
export function useCanvasData({ project, rootPath }: UseCanvasDataProps) {
  ...
  const { nodes, links } = useMemo(() => {
    const cNodes: CanvasNode[] = [];
    const cLinks: CanvasLink[] = [];
  ...
  }, [project, folderStack, rootPath]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return { nodes, links, displayNodes };
}
